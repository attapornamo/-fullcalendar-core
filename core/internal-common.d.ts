...
declare function createDuration(
  input: DurationInput,
  unit?: string
): Duration | null;
...
declare function multiplyDuration(
  d: Duration,
  n: number
): {
  years: number;
...
declare function wholeDivideDurations(
  numerator: Duration,
...
type FieldSpecInput<Subject> =
  | string
  | string[]
  | FieldSpecInputFunc<Subject>
  | FieldSpecInputFunc<Subject>[];
type FieldSpecInputFunc<Subject> = (a: Subject, b: Subject) => number;
declare function parseFieldSpecs<Subject>(
  input: FieldSpecInput<Subject>
): OrderSpec<Subject>[];
declare function compareByFieldSpecs<Subject>(
  obj0: Subject,
  obj1: Subject,
...
declare function intersectRanges(
  range0: OpenDateRange,
  range1: OpenDateRange
): OpenDateRange;
declare function rangesEqual(
  range0: OpenDateRange,
  range1: OpenDateRange
): boolean;
declare function rangesIntersect(
  range0: OpenDateRange,
  range1: OpenDateRange
): boolean;
declare function rangeContainsRange(
  outerRange: OpenDateRange,
  innerRange: OpenDateRange
): boolean;
declare function rangeContainsMarker(
  range: OpenDateRange,
...
declare function createEventInstance(
  defId: string,
  range: DateRange,
  forcedStartTzo?: number,
  forcedEndTzo?: number
...
declare function applyMutationToEventStore(
  eventStore: EventStore,
  eventConfigBase: EventUiHash,
  mutation: EventMutation,
  context: CalendarContext
): EventStore;
type eventDefMutationApplier = (
  eventDef: EventDef,
  mutation: EventMutation,
  context: CalendarContext
) => void;
...
declare function buildEventApis(
  eventStore: EventStore,
  context: CalendarContext,
  excludeInstance?: EventInstance
): EventImpl[];
...
interface DateProfileOptions {
  ...
  validRangeInput?:
    | DateRangeInput
    | ((this: CalendarImpl, nowDate: Date) => DateRangeInput);
  visibleRangeInput?:
    | DateRangeInput
    | ((this: CalendarImpl, nowDate: Date) => DateRangeInput);
...
declare class Emitter<HandlerFuncs extends HandlerFuncTypeHash> {
  ...
  on<Prop extends keyof HandlerFuncs>(
    type: Prop,
    handler: HandlerFuncs[Prop]
  ): void;
  off<Prop extends keyof HandlerFuncs>(
    type: Prop,
    handler?: HandlerFuncs[Prop]
  ): void;
  trigger<Prop extends keyof HandlerFuncs>(
    type: Prop,
    ...args: Parameters<HandlerFuncs[Prop]>
  ): void;
...
type CalendarDataBase = CalendarOptionsData &
...
declare class CalendarImpl implements CalendarApi {
  ...
  updateSize(): void;
  setOption<OptionName extends keyof CalendarOptions>(
    name: OptionName,
    val: CalendarOptions[OptionName]
  ): void;
  getOption<OptionName extends keyof CalendarOptions>(
    name: OptionName
  ): CalendarOptions[OptionName];
  getAvailableLocaleCodes(): string[];
  on<ListenerName extends keyof CalendarListeners>(
    handlerName: ListenerName,
    handler: CalendarListeners[ListenerName]
  ): void;
  off<ListenerName extends keyof CalendarListeners>(
    handlerName: ListenerName,
    handler: CalendarListeners[ListenerName]
  ): void;
  trigger<ListenerName extends keyof CalendarListeners>(
    handlerName: ListenerName,
  ...
  addEvent(
    ...
    sourceInput?: EventSourceImpl | string | boolean
  ): EventImpl | null;
...
type ReducerFunc = (
  // TODO: rename to CalendarDataInjector. move view-props-manip hook here as well?
...
declare class CalendarDataManager {
  ...
  private buildTitle;
  emitter: Emitter<
    Required<RefinedOptionsFromRefiners<Required<CalendarListenerRefiners>>>
  >;
...
declare function triggerDateSelect(
  selection: DateSpan,
  pev: PointerDragEvent | null,
  context: CalendarContext & {
    viewApi?: ViewImpl;
  }
): void;
...
declare function getDefaultEventEnd(
  allDay: boolean,
  marker: DateMarker,
  context: CalendarContext
): DateMarker;
...
declare function mapHash<InputItem, OutputItem>(
  hash: {
...
declare function compareObjs(
  oldProps: any,
  newProps: any,
  equalityFuncs?: EqualityFuncs<any>
): boolean;
declare function collectFromHash<Item>(
  hash: {
    [key: string]: Item;
  },
  startIndex?: number,
...
declare abstract class PureComponent<
  Props = Dictionary,
  State = Dictionary,
...
declare function addPropsEquality(
  this: {
    prototype: {
      propEquality: any;
    };
...
declare function addStateEquality(
  this: {
    prototype: {
      stateEquality: any;
    };
...
declare function setRef<RefType>(
...
declare function translateRect(
  rect: Rect,
  deltaX: number,
  deltaY: number
): Rect;
...
declare abstract class DateComponent<
  Props = Dictionary,
  State = Dictionary,
...
type eventDragMutationMassager = (
  mutation: EventMutation,
  hit0: Hit,
  hit1: Hit
) => void;
type EventDropTransformers = (
  mutation: EventMutation,
  context: CalendarContext
) => Dictionary;
type eventIsDraggableTransformer = (
  val: boolean,
  eventDef: EventDef,
  eventUi: EventUi,
  context: CalendarContext
...
type EventSourceInputObject = EventUiInput &
  RawOptionsFromRefiners<Required<EventSourceRefiners>>;
...
type EventSourceRefined = EventUiRefined &
  RefinedOptionsFromRefiners<Required<EventSourceRefiners>>;
...
interface RecurringType<RecurringData> {
  parse: (
    ...
    dateEnv: DateEnv
  ) => ParsedRecurring<RecurringData> | null;
  expand: (
    typeData: any,
...
declare function getSectionHasLiquidHeight(
  props: {
    liquid: boolean;
  },
  sectionConfig: SectionConfig
): boolean;
declare function getAllowYScrolling(
  props: {
    liquid: boolean;
  },
  sectionConfig: SectionConfig
): boolean;
declare function renderChunkContent(
  sectionConfig: SectionConfig,
...
declare function renderMicroColGroup(
  cols: ColProps[],
  shrinkWidth?: number
): VNode;
declare function sanitizeShrinkWidth(shrinkWidth?: number): number;
declare function hasShrinkWidth(cols: ColProps[]): boolean;
declare function getScrollGridClassNames(
  liquid: boolean,
  context: ViewContext
): any[];
declare function getSectionClassNames(
  sectionConfig: SectionConfig,
  wholeTableVGrow: boolean
): string[];
declare function renderScrollShim(
  arg: ChunkContentCallbackArgs
...
type ScrollGridImpl = {
  new (
    props: ScrollGridProps,
    context: ViewContext
  ): Component<ScrollGridProps>;
};
...
type EventInput = EventUiInput &
  RawOptionsFromRefiners<Required<EventRefiners>> & {
    // Required hack
    [extendedProp: string]: any;
  };
type EventRefined = EventUiRefined &
  RefinedOptionsFromRefiners<Required<EventRefiners>>;
...
declare function refineEventDef(
  raw: EventInput,
  context: CalendarContext,
...
declare function parseEventDef(
  refined: EventRefined,
  extra: Dictionary,
  sourceId: string,
  allDay: boolean,
...
declare function eventTupleToStore(
  tuple: EventTuple,
  eventStore?: EventStore
): EventStore;
declare function getRelevantEvents(
  eventStore: EventStore,
  instanceId: string
): EventStore;
declare function createEmptyEventStore(): EventStore;
declare function mergeEventStores(
  store0: EventStore,
  store1: EventStore
): EventStore;
...
declare function sliceEventStore(
  eventStore: EventStore,
  eventUiBases: EventUiHash,
  framingRange: DateRange,
  nextDayThreshold?: Duration
...
declare function sortEventSegs(
  segs: any,
  eventOrderSpecs: OrderSpec<EventImpl>[]
): Seg[];
...
declare function buildSegTimeText(
  seg: Seg,
...
declare function getSegAnchorAttrs(
  seg: Seg,
  context: ViewContext
):
  | {
...
declare function parseBusinessHours(
  input: BusinessHoursInput,
  context: CalendarContext
): EventStore;

type ElRef = Ref<HTMLElement>;
type ElAttrs = JSX.HTMLAttributes &
  JSX.SVGAttributes & {
    ref?: ElRef;
  } & Record<string, any>;
...
declare function buildElAttrs(
  props: ElAttrsProps,
  extraClassNames?: string[],
  elRef?: ElRef
): ElAttrs;

type ContentContainerProps<RenderProps> = ElAttrsProps &
...
declare class ContentContainer<RenderProps> extends Component<
...
type InnerContainerFunc<RenderProps> = (
...
declare const NowIndicatorContainer: (
  props: NowIndicatorContainerProps
) => createElement.JSX.Element;
...
declare const WeekNumberContainer: (
  props: WeekNumberContainerProps
) => createElement.JSX.Element;
...
declare class MoreLinkContainer extends BaseComponent<
...
declare class CustomRenderingStore<RenderProps> extends Store<
...
declare const BASE_OPTION_REFINERS: {
  navLinkDayClick: Identity<
    string | ((this: CalendarApi, date: Date, jsEvent: UIEvent) => void)
  >;
  navLinkWeekClick: Identity<
    string | ((this: CalendarApi, weekStart: Date, jsEvent: UIEvent) => void)
  >;
  duration: typeof createDuration;
  ...
  dropAccept: Identity<
    string | ((this: CalendarApi, draggable: any) => boolean)
  >;
  ...
  validRange: Identity<
    DateRangeInput | ((this: CalendarApi, nowDate: Date) => DateRangeInput)
  >;
  visibleRange: Identity<
    DateRangeInput | ((this: CalendarApi, currentDate: Date) => DateRangeInput)
  >;
...
type BaseOptions = RawOptionsFromRefiners<
  // as RawOptions
...
type BaseOptionsRefined = DefaultedRefinedOptions<
...
declare const CALENDAR_LISTENER_REFINERS: {
  ...
  windowResize: Identity<(arg: { view: ViewApi }) => void>;
...
type CalendarListenersLoose = RefinedOptionsFromRefiners<
  Required<CalendarListenerRefiners>
>;
...
type CalendarOptionsRefined = BaseOptionsRefined &
  CalendarListenersLoose &
  RefinedOptionsFromRefiners<Required<CalendarOptionRefiners>>;
...
type ViewOptions = BaseOptions &
  CalendarListenersLoose &
  RawOptionsFromRefiners<Required<ViewOptionRefiners>>;
type ViewOptionsRefined = BaseOptionsRefined &
  CalendarListenersLoose &
  RefinedOptionsFromRefiners<Required<ViewOptionRefiners>>;
declare function refineProps<
  Refiners extends GenericRefiners,
  Raw extends RawOptionsFromRefiners<Refiners>,
>(
  input: Raw,
  refiners: Refiners
): {
...
type RawOptionsFromRefiners<Refiners extends GenericRefiners> = {
  [Prop in keyof Refiners]?: Refiners[Prop] extends (
    input: infer RawType
  ) => infer RefinedType
    ? any extends RawType
      ? RefinedType
...
type DefaultedRefinedOptions<
  RefinedOptions extends Dictionary,
  DefaultKey extends keyof RefinedOptions,
...
declare function requestJson<ParsedResponse>(
  method: string,
  url: string,
  params: Dictionary
): Promise<[ParsedResponse, Response]>;

declare function computeVisibleDayRange(
...
declare function isArraysEqual(
  a0: any,
  a1: any,
  equalityFunc?: (v0: any, v1: any) => boolean
): boolean;

declare function memoize<Args extends any[], Res>(
  workerFunc: (...args: Args) => Res,
  resEquality?: (res0: Res, res1: Res) => boolean,
  teardownFunc?: (res: Res) => void
): (...args: Args) => Res;
declare function memoizeObjArg<Arg extends Dictionary, Res>(
...
declare function findElements(
  container: HTMLElement[] | HTMLElement | NodeListOf<HTMLElement>,
  selector: string
): HTMLElement[];
declare function findDirectChildren(
...
declare function buildNavLinkAttrs(
  context: ViewContext,
  dateMarker: DateMarker,
  viewType?: string,
  isTabbable?: boolean
...
declare function whenTransitionDone(
  el: HTMLElement,
...
declare function computeInnerRect(
  el: any,
  goWithinPadding?: boolean,
  doFromWindowViewport?: boolean
): {
  left: any;
  right: number;
  top: any;
...
declare function unpromisify<Res>(
  func: (
...
declare function buildIsoString(
  marker: DateMarker,
  timeZoneOffset?: number,
...
declare function binarySearch<Item>(
  a: Item[],
  searchVal: number,
...
declare class CalendarRoot extends BaseComponent<
  CalendarRootProps,
  CalendarRootState
> {
  state: {
...
declare function computeFallbackHeaderFormat(
  datesRepDistinctDays: boolean,
  dayCnt: number
): DateFormatter;
...
declare function isInteractionValid(
  interaction: EventInteractionState,
  dateProfile: DateProfile,
  context: CalendarContext
): boolean;
declare function isDateSelectionValid(
  dateSelection: DateSpan,
  dateProfile: DateProfile,
  context: CalendarContext
): boolean;
declare function isPropsValid(
...
declare class Scroller
  extends BaseComponent<ScrollerProps>
  implements ScrollerLike
{
  el: HTMLElement;
  render(): createElement.JSX.Element;
  handleEl: (el: HTMLElement) => void;
...
declare class SimpleScrollGrid extends BaseComponent<
...
type EventContainerProps = ElProps &
  MinimalEventProps & {
...
export {
  ViewContentArg as $,
  AllowFunc as A,
  BusinessHoursInput as B,
  CalendarImpl as C,
  DateInput as D,
  EventSourceApi as E,
  FormatterInput as F,
  WeekNumberMountArg as G,
  MoreLinkMountArg as H,
  SlotLaneContentArg as I,
  JsonRequestError as J,
  SlotLaneMountArg as K,
  LocaleInput as L,
  MoreLinkContentArg as M,
  NativeFormatterOptions as N,
  OverlapFunc as O,
  PluginDefInput as P,
  SlotLabelContentArg as Q,
  SlotLabelMountArg as R,
  SpecificViewContentArg as S,
  AllDayContentArg as T,
  AllDayMountArg as U,
  ViewApi as V,
  WillUnmountHandler as W,
  DayHeaderContentArg as X,
  DayHeaderMountArg as Y,
  DayCellContentArg as Z,
  DayCellMountArg as _,
  CalendarOptions as a,
  disableCursor as a$,
  ViewMountArg as a0,
  EventClickArg as a1,
  EventHoveringArg as a2,
  DateSelectArg as a3,
  DateUnselectArg as a4,
  WeekNumberCalculation as a5,
  ToolbarInput as a6,
  CustomButtonInput as a7,
  ButtonIconsInput as a8,
  ButtonTextCompoundInput as a9,
  CalendarListenerRefiners as aA,
  BASE_OPTION_DEFAULTS as aB,
  identity as aC,
  refineProps as aD,
  EventDef as aE,
  EventDefHash as aF,
  EventInstance as aG,
  EventInstanceHash as aH,
  createEventInstance as aI,
  EventRefined as aJ,
  EventTuple as aK,
  EventRefiners as aL,
  parseEventDef as aM,
  refineEventDef as aN,
  parseBusinessHours as aO,
  OrderSpec as aP,
  padStart as aQ,
  isInt as aR,
  parseFieldSpecs as aS,
  compareByFieldSpecs as aT,
  flexibleCompare as aU,
  preventSelection as aV,
  allowSelection as aW,
  preventContextMenu as aX,
  allowContextMenu as aY,
  compareNumbers as aZ,
  enableCursor as a_,
  EventContentArg as aa,
  EventMountArg as ab,
  DatesSetArg as ac,
  EventAddArg as ad,
  EventChangeArg as ae,
  EventDropArg as af,
  EventRemoveArg as ag,
  ButtonHintCompoundInput as ah,
  CustomRenderingHandler as ai,
  CustomRenderingStore as aj,
  DateSpanApi as ak,
  DatePointApi as al,
  DateSelectionApi as am,
  Duration as an,
  EventSegment as ao,
  MoreLinkAction as ap,
  MoreLinkSimpleAction as aq,
  MoreLinkArg as ar,
  MoreLinkHandler as as,
  Identity as at,
  Dictionary as au,
  BaseOptionRefiners as av,
  BaseOptionsRefined as aw,
  ViewOptionsRefined as ax,
  RawOptionsFromRefiners as ay,
  RefinedOptionsFromRefiners as az,
  PluginDef as b,
  rangeContainsRange as b$,
  guid as b0,
  computeVisibleDayRange as b1,
  isMultiDayRange as b2,
  diffDates as b3,
  removeExact as b4,
  isArraysEqual as b5,
  MemoizeHashFunc as b6,
  MemoiseArrayFunc as b7,
  memoize as b8,
  memoizeObjArg as b9,
  createEmptyEventStore as bA,
  mergeEventStores as bB,
  getRelevantEvents as bC,
  eventTupleToStore as bD,
  EventUiHash as bE,
  EventUi as bF,
  combineEventUis as bG,
  createEventUi as bH,
  SplittableProps as bI,
  Splitter as bJ,
  getDayClassNames as bK,
  getDateMeta as bL,
  getSlotClassNames as bM,
  buildNavLinkAttrs as bN,
  preventDefault as bO,
  whenTransitionDone as bP,
  computeInnerRect as bQ,
  computeEdges as bR,
  getClippingParents as bS,
  computeRect as bT,
  unpromisify as bU,
  Emitter as bV,
  DateRange as bW,
  rangeContainsMarker as bX,
  intersectRanges as bY,
  rangesEqual as bZ,
  rangesIntersect as b_,
  memoizeArraylike as ba,
  memoizeHashlike as bb,
  Rect as bc,
  Point as bd,
  intersectRects as be,
  pointInsideRect as bf,
  constrainPoint as bg,
  getRectCenter as bh,
  diffPoints as bi,
  translateRect as bj,
  mapHash as bk,
  ...
  isPropsEqual as bm,
  compareObjs as bn,
  collectFromHash as bo,
  findElements as bp,
  findDirectChildren as bq,
  removeElement as br,
  applyStyle as bs,
  elementMatches as bt,
  elementClosest as bu,
  getEventTargetViaRoot as bv,
  getUniqueDomId as bw,
  parseClassNames as bx,
  getCanVGrowWithinCell as by,
  EventStore as bz,
  CalendarApi as c,
  Interaction as c$,
  PositionCache as c0,
  ScrollController as c1,
  ElementScrollController as c2,
  WindowScrollController as c3,
  Theme as c4,
  ViewContext as c5,
  ViewContextType as c6,
  Seg as c7,
  EventSegUiInteractionState as c8,
  DateComponent as c9,
  greatestDurationDenominator as cA,
  DateEnv as cB,
  createFormatter as cC,
  DateFormatter as cD,
  VerboseFormattingArg as cE,
  formatIsoTimeString as cF,
  formatDayString as cG,
  buildIsoString as cH,
  formatIsoMonthStr as cI,
  NamedTimeZoneImpl as cJ,
  parse as cK,
  EventSourceDef as cL,
  EventSourceRefined as cM,
  EventSourceRefiners as cN,
  SegSpan as cO,
  SegRect as cP,
  SegEntry as cQ,
  SegInsertion as cR,
  ...
  SegHierarchy as cT,
  buildEntryKey as cU,
  getEntrySpanEnd as cV,
  binarySearch as cW,
  groupIntersectingEntries as cX,
  intersectSpans as cY,
  InteractionSettings as cZ,
  InteractionSettingsStore as c_,
  CalendarData as ca,
  ViewProps as cb,
  DateProfile as cc,
  DateProfileGenerator as cd,
  ViewSpec as ce,
  DateSpan as cf,
  isDateSpansEqual as cg,
  DateMarker as ch,
  addDays as ci,
  startOfDay as cj,
  addMs as ck,
  addWeeks as cl,
  diffWeeks as cm,
  diffWholeWeeks as cn,
  diffWholeDays as co,
  diffDayAndTime as cp,
  diffDays as cq,
  isValidDate as cr,
  createDuration as cs,
  asCleanDays as ct,
  multiplyDuration as cu,
  addDurations as cv,
  asRoughMinutes as cw,
  asRoughSeconds as cx,
  asRoughMs as cy,
  wholeDivideDurations as cz,
  EventApi as d,
  getAllowYScrolling as d$,
  interactionSettingsToStore as d0,
  interactionSettingsStore as d1,
  PointerDragEvent as d2,
  Hit as d3,
  dateSelectionJoinTransformer as d4,
  eventDragMutationMassager as d5,
  EventDropTransformers as d6,
  ElementDragging as d7,
  config as d8,
  RecurringType as d9,
  Slicer as dA,
  EventMutation as dB,
  applyMutationToEventStore as dC,
  Constraint as dD,
  isPropsValid as dE,
  isInteractionValid as dF,
  isDateSelectionValid as dG,
  requestJson as dH,
  BaseComponent as dI,
  setRef as dJ,
  DelayedRunner as dK,
  ScrollGridProps as dL,
  ScrollGridSectionConfig as dM,
  ColGroupConfig as dN,
  ScrollGridChunkConfig as dO,
  SimpleScrollGridSection as dP,
  SimpleScrollGrid as dQ,
  ScrollerLike as dR,
  ColProps as dS,
  ChunkContentCallbackArgs as dT,
  ChunkConfigRowContent as dU,
  ChunkConfigContent as dV,
  hasShrinkWidth as dW,
  renderMicroColGroup as dX,
  getScrollGridClassNames as dY,
  getSectionClassNames as dZ,
  getSectionHasLiquidHeight as d_,
  DragMetaInput as da,
  DragMeta as db,
  parseDragMeta as dc,
  ViewPropsTransformer as dd,
  Action as de,
  CalendarContext as df,
  CalendarContentProps as dg,
  CalendarRoot as dh,
  DayHeader as di,
  computeFallbackHeaderFormat as dj,
  TableDateCell as dk,
  TableDowCell as dl,
  DaySeriesModel as dm,
  EventInteractionState as dn,
  sliceEventStore as dp,
  hasBgRendering as dq,
  getElSeg as dr,
  buildSegTimeText as ds,
  sortEventSegs as dt,
  getSegMeta as du,
  buildEventRangeKey as dv,
  getSegAnchorAttrs as dw,
  DayTableCell as dx,
  DayTableModel as dy,
  SlicedProps as dz,
  EventRenderRange as e,
  renderChunkContent as e0,
  computeShrinkWidth as e1,
  sanitizeShrinkWidth as e2,
  isColPropsEqual as e3,
  renderScrollShim as e4,
  getStickyFooterScrollbar as e5,
  getStickyHeaderDates as e6,
  OverflowValue as e7,
  Scroller as e8,
  getScrollbarWidths as e9,
  buildEventApis as eA,
  ElProps as eB,
  buildElAttrs as eC,
  InnerContainerFunc as eD,
  ContentContainer as eE,
  CustomRendering as eF,
  RefMap as ea,
  getIsRtlScrollbarOnLeft as eb,
  NowTimer as ec,
  ScrollRequest as ed,
  ScrollResponder as ee,
  MountArg as ef,
  StandardEvent as eg,
  NowIndicatorContainer as eh,
  DayCellContainer as ei,
  hasCustomDayCellContent as ej,
  MinimalEventProps as ek,
  EventContainer as el,
  renderFill as em,
  BgEvent as en,
  WeekNumberContainerProps as eo,
  WeekNumberContainer as ep,
  MoreLinkContainer as eq,
  computeEarliestSegStart as er,
  ViewContainerProps as es,
  ViewContainer as et,
  DatePointTransform as eu,
  DateSpanTransform as ev,
  triggerDateSelect as ew,
  getDefaultEventEnd as ex,
  injectStyles as ey,
  EventImpl as ez,
  CalendarListeners as f,
  DurationInput as g,
  DateSpanInput as h,
  DateRangeInput as i,
  EventSourceInput as j,
  EventSourceFunc as k,
  EventSourceFuncArg as l,
  EventInput as m,
  EventInputTransformer as n,
  CssDimValue as o,
  LocaleSingularArg as p,
  ConstraintInput as q,
  ViewComponentType as r,
  sliceEvents as s,
  SpecificViewMountArg as t,
  ClassNamesGenerator as u,
  CustomContentGenerator as v,
  DidMountHandler as w,
  NowIndicatorContentArg as x,
  NowIndicatorMountArg as y,
  WeekNumberContentArg as z,
};
