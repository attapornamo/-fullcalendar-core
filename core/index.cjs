...
function buildBuildPluginHooks() {
  ...
  return (overrideDefs, globalDefs) => {
    if (
      !currentHooks ||
      !internalCommon.isArraysEqual(overrideDefs, currentOverrideDefs) ||
      !internalCommon.isArraysEqual(globalDefs, currentGlobalDefs)
    ) {
...
function parseViewConfig(input) {
  ...
  if (rawOptions.content) {
    ...
    component &&
    !(component.prototype instanceof internalCommon.BaseComponent)
  ) {
...
function createViewHookComponent(options) {
  return (viewProps) =>
    preact.createElement(
      internalCommon.ViewContextType.Consumer,
      null,
      (context) =>
        preact.createElement(internalCommon.ContentContainer, {
          elTag: 'div',
...
function buildViewSpecs(
  ...
  return internalCommon.mapHash(viewDefs, (viewDef) =>
    buildViewSpec(
      viewDef,
      overrideConfigs,
      optionOverrides,
...
function removeSource(eventSourceHash, sourceId) {
  return internalCommon.filterHash(
    eventSourceHash,
...
function fetchDirtySources(sourceHash, fetchRange, context) {
  return fetchSourcesByIds(
    sourceHash,
    internalCommon.filterHash(sourceHash, (eventSource) =>
...
function excludeStaticSources(eventSources, context) {
  return internalCommon.filterHash(eventSources, (eventSource) =>
  ...
  let widgets = sectionSubstrs.map((buttonGroupStr) =>
    buttonGroupStr.split(',').map((buttonName) => {
      ...
      if ((customButtonProps = calendarCustomButtons[buttonName])) {
        ...
        buttonHint = internalCommon.formatWithOrdinals(
        ...
        if (buttonName === 'prevYear' || buttonName === 'nextYear') {
          ...
          buttonHint = (navUnit) =>
            internalCommon.formatWithOrdinals(
...
let eventSourceDef$1 = {
  ...
  fetch(arg, successCallback, errorCallback) {
    ...
    internalCommon.unpromisify(
      func.bind(
        null,
        internalCommon.buildRangeApiWithTimeZone(arg.range, dateEnv)
      ),
...
let eventSourceDef = {
  ...
  fetch(arg, successCallback, errorCallback) {
    ...
    internalCommon
      .requestJson(meta.method, meta.url, requestParams)
      .then(([rawEvents, response]) => {
  ...
  parse(refined, dateEnv) {
    if (
      ...
      if (!duration && refined.startTime && refined.endTime) {
        duration = internalCommon.subtractDurations(
          refined.endTime,
          refined.startTime
        );
      }
...
function handleEventSources(inputs, context) {
  let unfoundSources = internalCommon.hashValuesToArray(
...
function handleEventStore(eventStore, context) {
  ...
  if (emitter.hasHandlers('eventsSet')) {
    emitter.trigger(
      'eventsSet',
      internalCommon.buildEventApis(eventStore, context)
    );
  }
}
...
class TaskRunner {
  constructor(runTaskOption, drainedOption) {
    ...
    this.queue = [];
    this.delayedRunner = new internalCommon.DelayedRunner(
...
function buildTitle(dateProfile, viewOptions, dateEnv) {
  ...
  return dateEnv.formatRange(
    range.start,
    range.end,
    internalCommon.createFormatter(
...
function buildTitleFormat(dateProfile) {
  ...
  let days = internalCommon.diffWholeDays(
...
class CalendarDataManager {
  constructor(props) {
    this.computeCurrentViewData = internalCommon.memoize(
    ...
    this.buildEventUiBySource = internalCommon.memoize(
      buildEventUiBySource,
      internalCommon.isPropsEqual
    );
    this.buildEventUiBases = internalCommon.memoize(buildEventUiBases);
    this.parseContextBusinessHours = internalCommon.memoizeObjArg(
    ...
    let currentDate = internalCommon.getInitialDate(
      optionsData.calendarOptions,
      optionsData.dateEnv
    );
    let dateProfile = currentViewData.dateProfileGenerator.build(currentDate);
    if (
      !internalCommon.rangeContainsMarker(dateProfile.activeRange, currentDate)
    ) {
  ...
  _handleAction(action) {
    ...
    let eventStore = internalCommon.reduceEventStore(
      state.eventStore,
      action,
      eventSources,
      dateProfile,
    ...
    if (oldCalendarOptions && oldCalendarOptions !== newCalendarOptions) {
      if (oldCalendarOptions.timeZone !== newCalendarOptions.timeZone) {
        ...
        state.eventStore = data.eventStore =
          internalCommon.rezoneEventStoreDates(
            data.eventStore,
            oldData.dateEnv,
            data.dateEnv
          );
  ...
  processRawCalendarOptions(optionOverrides, dynamicOptionOverrides) {
    ...
    let refiners = (this.currentCalendarOptionsRefiners = Object.assign(
      Object.assign(
        Object.assign(
          Object.assign(
            Object.assign({}, internalCommon.BASE_OPTION_REFINERS),
            internalCommon.CALENDAR_LISTENER_REFINERS
          ),
          internalCommon.CALENDAR_OPTION_REFINERS
        ),
    ...
    let refiners = Object.assign(
      Object.assign(
        Object.assign(
          ...
          internalCommon.VIEW_OPTION_REFINERS
        ),
        pluginHooks.listenerRefiners
      ),
      pluginHooks.optionRefiners
...
function buildDateProfileGenerator(props) {
  let DateProfileGeneratorClass =
    props.dateProfileGeneratorClass || internalCommon.DateProfileGenerator;
...
function parseContextBusinessHours(calendarContext) {
  return internalCommon.parseBusinessHours(
  ...
  constructor(settings) {
    ...
    this.handleSegClick = (ev, segEl) => {
      ...
      if (
        ...
        let hasUrlContainer = internalCommon.elementClosest(
          ev.target,
          '.fc-event-forced-url'
        );
        ...
        context.emitter.trigger('eventClick', {
          el: segEl,
          event: new internalCommon.EventImpl(
            component.context,
            seg.eventRange.def,
...
class EventHovering extends internalCommon.Interaction {
  constructor(settings) {
    ...
    this.removeHoverListeners = internalCommon.listenToHoverBySelector(
      settings.el,
      '.fc-event', // on both fg and bg events
      this.handleSegEnter,
      this.handleSegLeave
...
class CalendarContent extends internalCommon.PureComponent {
  constructor() {
    super(...arguments);
    this.buildViewContext = internalCommon.memoize(
      internalCommon.buildViewContext
    );
    this.buildViewPropTransformers = internalCommon.memoize(
    ...
    this.registerInteractiveComponent = (component, settingsInput) => {
      let settings = internalCommon.parseInteractionSettings(
        component,
        settingsInput
      );
  ...
  render() {
    ...
    let toolbarProps = this.buildToolbarProps(
      ...
      props.currentDate,
      internalCommon.getNow(props.options.now, props.dateEnv), // TODO: use NowTimer????
      props.viewTitle
    );
    let viewVGrow = false;
    let viewHeight = '';
    ...
    let viewLabelId =
      toolbarConfig.header && toolbarConfig.header.hasTitle
        ...
        : undefined;
    return preact.createElement(
      internalCommon.ViewContextType.Provider,
      { value: viewContext },
...
function buildToolbarProps(
  ...
  return {
    ...
    isTodayEnabled:
      todayInfo.isValid &&
      !internalCommon.rangeContainsMarker(dateProfile.currentRange, now),
...
class Calendar extends internalCommon.CalendarImpl {
  constructor(el, optionOverrides = {}) {
          ...
          preact.render(
            preact.createElement(
              ...
              (classNames, height, isHeightAuto, forPrint) => {
                ...
                return preact.createElement(
                  internalCommon.RenderId.Provider,
    ...
    this.el = el;
    this.renderRunner = new internalCommon.DelayedRunner(
      this.handleRenderRequest
    );
...
function sliceEvents(props, allDay) {
  return internalCommon.sliceEventStore(
    props.eventStore,
    props.eventUiBases,
